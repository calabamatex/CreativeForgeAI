...
class User(Base):
    ...
    brand_guidelines: Mapped[list["BrandGuideline"]] = relationship(back_populates="creator", lazy="raise")
    campaigns: Mapped[list["Campaign"]] = relationship(back_populates="creator", lazy="raise")
...
class BrandGuideline(Base):
    ...
    primary_colors: Mapped[dict | None] = mapped_column(JSONB, server_default="[]")
    secondary_colors: Mapped[dict | None] = mapped_column(JSONB, server_default="[]")
    primary_font: Mapped[str | None] = mapped_column(String(100), server_default="Arial")
    ...
    prohibited_elements: Mapped[dict | None] = mapped_column(JSONB, server_default="[]")
    logo_config: Mapped[dict | None] = mapped_column(JSONB, server_default="{}")
    text_customization: Mapped[dict | None] = mapped_column(JSONB)
    post_processing: Mapped[dict | None] = mapped_column(JSONB)
    raw_extracted_data: Mapped[dict | None] = mapped_column(JSONB)
...
class Campaign(Base):
    ...
    image_backend: Mapped[str] = mapped_column(String(50), nullable=False, server_default="firefly")
    target_locales: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default='["en-US"]')
    aspect_ratios: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default='["1:1","9:16","16:9"]')
    brand_guidelines_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("brand_guidelines.id"))
    localization_guidelines: Mapped[dict | None] = mapped_column(JSONB)
    legal_guidelines: Mapped[dict | None] = mapped_column(JSONB)
    created_by: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
    ...
    brand_guidelines: Mapped[Optional["BrandGuideline"]] = relationship(back_populates="campaigns", lazy="selectin")
    creator: Mapped[Optional["User"]] = relationship(back_populates="campaigns", lazy="selectin")
    ...
    jobs: Mapped[list["Job"]] = relationship(back_populates="campaign", lazy="raise")
...
class CampaignProduct(Base):
    ...
    key_features: Mapped[dict | None] = mapped_column(JSONB, server_default="[]")
    generation_prompt: Mapped[str | None] = mapped_column(Text)
    hero_image_path: Mapped[str | None] = mapped_column(String(500))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())

    # Relationships
    campaign: Mapped["Campaign"] = relationship(back_populates="products", lazy="raise")
...
class GeneratedAsset(Base):
    ...
    # Relationships
    campaign: Mapped["Campaign"] = relationship(back_populates="assets", lazy="raise")
...
class Job(Base):
    ...
    # Relationships
    campaign: Mapped["Campaign"] = relationship(back_populates="jobs", lazy="raise")
...
class ComplianceReport(Base):
    ...
    violations: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default="[]")
    ...
    # Relationships
    campaign: Mapped["Campaign"] = relationship(back_populates="compliance_reports", lazy="raise")
...
class CampaignMetric(Base):
    ...
    technical_metrics: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default="{}")
    business_metrics: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default="{}")
    recorded_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())

    # Relationships
    campaign: Mapped["Campaign"] = relationship(back_populates="metrics", lazy="raise")
