...
class OpenAIImageService(ImageGenerationService):
    ...
    def __init__(self, api_key: str | None = None, max_retries: int = 3):
        config = get_config()
        super().__init__(api_key=api_key or config.OPENAI_API_KEY, max_retries=max_retries)
            ...
            try:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.warning("openai.api_error", status=response.status, error=error_text)
