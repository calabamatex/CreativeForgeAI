...
class Config:
    ...
    def __init__(self):
        ...
        # ---------------------------------------------------------------
        self.TRUST_FORWARDED_FOR: bool = os.getenv("TRUST_FORWARDED_FOR", "false").lower() == "true"
        self.TRUSTED_PROXIES: set[str] = {p.strip() for p in os.getenv("TRUSTED_PROXIES", "").split(",") if p.strip()}
    ...
    def estimate_image_cost_usd(self, backend: str | None, api_calls: int) -> float:
    ...
    def validate(self) -> tuple[bool, list[str]]:
        ...
        if self.ENABLE_CLAUDE_INTEGRATION and not self.CLAUDE_API_KEY:
            warnings.append(
                "ANTHROPIC_API_KEY not set - guideline extraction and localization will use fallback methods"
            )
