...
class ImageProcessorV2:
    ...
    def resize_to_aspect_ratio(self, image_bytes: bytes, target_ratio: str) -> Image.Image:
        """Resize image to target aspect ratio."""
        image = Image.open(BytesIO(image_bytes))

        ratio_map = {"1:1": (1024, 1024), "9:16": (1080, 1920), "16:9": (1920, 1080), "4:5": (1080, 1350)}
    ...
    def apply_text_overlay(
        self, image: Image.Image, message: CampaignMessage, brand_guidelines: ComprehensiveBrandGuidelines | None = None
    ) -> Image.Image:
        ...
        for element_name, text, y_ratio, base_size_ratio in elements:
            ...
            # Fit text to width
            font, final_text = self._fit_text_to_width(text, base_font_size, max_text_width, style.font_weight)
    ...
    def _get_text_element_style(
        self, element_name: str, brand_guidelines: ComprehensiveBrandGuidelines | None
    ) -> TextElementStyle:
        """
        ...
        default_style = TextElementStyle(
            color="#FFFFFF", shadow=TextShadow(enabled=True, color="#000000", offset_x=2, offset_y=2)
        )
        ...
        if brand_guidelines.text_shadow:
            legacy_shadow = TextShadow(enabled=True, color=brand_guidelines.text_shadow_color, offset_x=2, offset_y=2)
        ...
        return TextElementStyle(color=brand_guidelines.text_color, shadow=legacy_shadow, background=legacy_background)
    ...
    def _render_text_element(
        ...
        if style.background and style.background.enabled:
            img = self._draw_background_box(img, x_pos, y_pos, text_width, text_height, style.background)
    ...
    def _draw_background_box(
        self, img: Image.Image, x: int, y: int, text_width: int, text_height: int, background: TextBackgroundBox
    ) -> Image.Image:
    ...
    def _draw_text_outline(
        self, draw: ImageDraw.ImageDraw, text: str, x: int, y: int, font: ImageFont.FreeTypeFont, outline: TextOutline
    ):
        ...
        for offset_x in range(-width, width + 1):
            for offset_y in range(-width, width + 1):
                ...
                if offset_x == 0 and offset_y == 0:
                    continue
                draw.text((x + offset_x, y + offset_y), text, fill=outline.color, font=font)

    def _calculate_x_position(self, img: Image.Image, text: str, font: ImageFont.FreeTypeFont, align: str) -> int:
    ...
    def _fit_text_to_width(
        self, text: str, initial_size: int, max_width: int, weight: str
