...
from src.models.enums import CampaignMessage, Product
...
class CampaignBrief(BaseModel):
    ...
    target_market: str | None = Field(default="Global", description="Target market")
    target_audience: str | None = Field(default="General", description="Target audience")
    ...
    aspect_ratios: list[str] = Field(default=["1:1", "9:16", "16:9"], description="Target aspect ratios")
    output_formats: list[str] = Field(default=["png", "jpg"], description="Output image formats")
    image_generation_backend: str = Field(
        default="firefly", description="Image generation backend: 'firefly', 'openai', 'gemini', or 'claude'"
    )
    brand_guidelines_file: str | None = Field(default=None, description="Path to brand guidelines document")
    localization_guidelines_file: str | None = Field(
        default=None, description="Path to localization guidelines document"
    )
    legal_compliance_file: str | None = Field(default=None, description="Path to legal compliance guidelines document")
    enable_localization: bool = Field(default=False, description="Enable multi-locale generation")
    target_locales: list[str] = Field(default_factory=lambda: ["en-US"], description="Target locales for campaign")
    ...
    def validate_backend(cls, v):
        ...
        if v.lower() not in valid_backends:
            raise ValueError(f"Invalid image generation backend: {v}. Must be one of {valid_backends}")
        return v.lower()
...
class CampaignOutput(BaseModel):
    ...
    generated_assets: list[GeneratedAsset] = Field(default_factory=list, description="List of all generated assets")
    ...
    generation_timestamp: datetime = Field(default_factory=datetime.now, description="Generation completion timestamp")

    # Enhanced metrics (v1.3.0+)
    technical_metrics: TechnicalMetrics | None = Field(default=None, description="Advanced technical metrics")
