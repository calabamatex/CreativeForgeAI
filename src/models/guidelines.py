...
from src.models.text_styles import PostProcessingConfig, TextCustomization
...
class ComprehensiveBrandGuidelines(BaseModel):
    ...
    secondary_colors: list[str] | None = Field(default_factory=list, description="Secondary colors")
    primary_font: str = Field(default="Arial", description="Primary font family")
    secondary_font: str | None = Field(default=None, description="Secondary font family")
    ...
    logo_placement: str = Field(
        default="bottom-right", description="Logo position: top-left, top-right, bottom-left, bottom-right"
    )
    ...
    logo_scale: float = Field(
        default=0.15, ge=0.05, le=0.5, description="Logo size as percentage of image width (0.05-0.5)"
    )
    ...
    text_customization: TextCustomization | None = Field(default=None, description="Per-element text styling")

    # NEW: Post-processing configuration (Phase 1)
    post_processing: PostProcessingConfig | None = Field(default=None, description="Image post-processing settings")
...
class LegalComplianceGuidelines(BaseModel):
    ...
    # Prohibited content
    prohibited_words: list[str] = Field(default_factory=list, description="Words that must not appear in any content")
    prohibited_phrases: list[str] = Field(
        default_factory=list, description="Phrases that must not appear in any content"
    )

    # Required disclaimers
    required_disclaimers: dict[str, str] = Field(
        default_factory=dict, description="Required disclaimer text by category (e.g., 'financial', 'health')"
    )
    ...
    restricted_terms: dict[str, list[str]] = Field(
        default_factory=dict, description="Terms that require specific context or disclaimers"
    )

    # Age and audience restrictions
    age_restrictions: int | None = Field(default=None, description="Minimum age requirement (e.g., 18 for alcohol)")
    restricted_audiences: list[str] = Field(
        default_factory=list, description="Audiences that cannot be targeted (e.g., 'children', 'minors')"
    )

    # Geographic restrictions
    restricted_regions: list[str] = Field(
        default_factory=list, description="Regions/countries where content is prohibited"
    )

    # Industry-specific compliance
    industry_regulations: list[str] = Field(
        default_factory=list, description="Applicable regulations (e.g., 'FDA', 'FTC', 'GDPR', 'COPPA')"
    )

    # Trademark and copyright
    protected_trademarks: list[str] = Field(
        default_factory=list, description="Competitor trademarks that must not be used"
    )

    # Content standards
    require_substantiation: bool = Field(default=False, description="Whether claims require scientific substantiation")
    prohibit_superlatives: bool = Field(
        default=False, description="Whether superlatives like 'best', 'perfect' are prohibited"
    )

    # Locale-specific restrictions
    locale_restrictions: dict[str, dict[str, Any]] = Field(
        default_factory=dict, description="Locale-specific legal restrictions"
    )
...
class LocalizationGuidelines(BaseModel):
    ...
    market_specific_rules: dict[str, dict[str, Any]] = Field(
        default_factory=dict, description="Market-specific rules and preferences"
    )
    prohibited_terms: dict[str, list[str]] = Field(default_factory=dict, description="Prohibited terms per locale")
    translation_glossary: dict[str, dict[str, str]] = Field(
        default_factory=dict, description="Translation glossary per locale"
    )
    tone_guidelines: dict[str, str] | None = Field(default_factory=dict, description="Tone guidelines per locale")
    cultural_considerations: dict[str, list[str]] | None = Field(
        default_factory=dict, description="Cultural considerations per locale"
    )
