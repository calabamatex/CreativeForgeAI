...
@click.option(
    "--backend",
    type=click.Choice(["firefly", "openai", "gemini", "dalle", "imagen"], case_sensitive=False),
    help="Override image generation backend",
)
...
def process(brief: str, backend: str, verbose: bool, dry_run: bool):
    ...
    try:
        ...
        if verbose:
            click.echo("✓ Brief validated successfully")
...
def new_campaign(campaign_id, campaign_name, interactive, output, **kwargs):
    ...
    try:
        ...
        output_path = generator.generate(
            campaign_id=campaign_id, campaign_name=campaign_name, output_path=output, interactive=interactive, **params
        )

        click.echo(f"\n✅ Campaign brief created: {output_path}")
        click.echo("\n📝 Next steps:")
        click.echo("   1. Edit the file to customize products and settings")
